...
interface QuotationData {
  ...
  photoOutput?: string;
  rawOutput?: string;
  // optional schedule for multi-date services
  schedule?: {
    type?: string;
    serviceGiven?: string;
    serviceType?: string;
    ...
    >
      ...
      <div className="flex justify-between items-start mb-6">
        ...
        <div className="flex items-center">
          ...
          <div>
            <h1 className="text-lg font-bold">{data.company?.name || 'Abeer Motion Picture Pvt. Ltd.'}</h1>
            ...
            {data.client.gstin && <p className="text-sm">GSTIN: {data.client.gstin}</p>}
          </div>
        </div>
        ...
        <div className="text-right border border-gray-300 p-3">
          ...
          {data.branch?.name && (
            <div className="mt-2 text-sm">Branch: {data.branch.name} {data.branch.code ? `(${data.branch.code})` : ''}</div>
          )}
          <div className="mt-2 border-t pt-2">
            <div className="font-bold">Received: ₹ {data.receivedAmount?.toLocaleString() || "0"}/-</div>
            <div className="text-xs text-gray-600">(Advance/received amounts shown)</div>
          </div>
        </div>
      </div>

      {/* Schedule at top if present */}
            ...
            {data.schedule.map((s, i) => (
              <div key={i} className="flex justify-between items-start">
                <div>
                  ...
                  <div className="text-xs text-gray-600">Qty: {s.quantity ?? 0} × Price: {s.price ? `₹ ${s.price.toLocaleString()}` : '-'} = <strong>₹ {Number(s.amount ?? 0).toLocaleString()}</strong></div>
                  <div className="text-xs">{s.venue?.name || ''} {s.venue?.address ? `- ${s.venue.address}` : ''}</div>
                </div>
      ...
      <div className=" mb-6">
          ...
          <div key={index} className="last:border-b-0">
            <div className="grid grid-cols-4 p-2 min-h-[60px]">
              <div className="pr-2">
                  ...
                  {item.serviceType && (
                    <div className="text-xs text-gray-600">{item.serviceType}</div>
                  )}
              </div>
        ...
        {/* Schedule quick view (if present) */}
        {data.schedule && data.schedule.length > 0 && (
            ...
            <div className="text-sm text-gray-700">
              {data.schedule.map((s, i) => (
                <div key={i} className="mb-2">
                  ...
                  <div className="text-xs">Qty: {s.quantity ?? 0} × Price: {s.price ? `₹ ${s.price.toLocaleString()}` : '-'} = ₹ {Number(s.amount ?? 0).toLocaleString()}</div>
                </div>
                ))}
            </div>
          </div>
