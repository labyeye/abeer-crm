...
const QuotationManagement = () => {
  ...
  const emptyScheduleEntry = () => ({
    type: "",
    // new field requested by user: serviceGiven describes what service will be given at this schedule
    serviceGiven: "",
    // pricing moved into schedule: quantity, price, amount
    quantity: 1,
    price: 0,
    amount: 0,
    serviceType: "",
  ...
  const [createData, setCreateData] = useState<any>({
    client: "",
    branch: "",
    // services: each service line now includes an editable `amount` (auto-calculated as quantity*price by default)
    services: [{ name: "", serviceType: "", quantity: 1, price: 0, amount: 0 }],
    gstPct: 0,
    notes: "",
    // servicesSchedule holds multiple function entries
  ...
  const handleQuotationAction = (action: string, quotationId: string) => {
    if (action === "Edit") {
      ...
      if (quotation) {
        ...
        // Populate servicesSchedule from existing quotation: prefer functionDetailsList, else use functionDetails
        ...
        setCreateData({
          ...
          // normalize existing services to ensure amount and serviceType exist
          services:
  ...
  const handleDownloadPDF = async (quotation: any) => {
    try {
      ...
      // Build dates/items mapping: prefer functionDetailsList, else use functionDetails
      ...
      const pdfData = {
        ...
        photoOutput: quotation.photoOutput || "",
        rawOutput: quotation.rawOutput || "",
        // include schedule for PDF rendering
        ...
        // include dates from functionDetailsList into each service item if service.dates provided by backend
        // Prefer building items from services if present, else use schedule-derived items
        items:
  ...
  const handleCreateSubmit = async () => {
    try {
      // Build services from schedule entries — pricing now stored on schedule
      ...
      // Build functionDetailsList from servicesSchedule and include backward-compatible functionDetails (first entry)
      ...
      const payload = {
        ...
        // include both array and legacy single field for compatibility
  ...
  // service line helpers removed — pricing now lives on schedule entries
          ...
          <div className="bg-white rounded-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            ...
            >
              ...
              <div>
                ...
                {/* Pricing is now part of each schedule entry; removed separate Services & Pricing section */}
