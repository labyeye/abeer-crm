...
const DailyExpenses = () => {
  ...
  const getBranchName = (branch: any) => {
    if (!branch) return '-';
    // branch may already be an object
    if (typeof branch === 'object') return branch.name || branch.companyName || branch.code || '-';
    // otherwise branch is likely an id string - find in loaded branches
  ...
  useEffect(() => {
    ...
    // If chairman, fetch list of branches for selection.
    // If non-chairman, preselect user's branch (if available).
    if (user) {
  ...
  const fetchExpenses = async () => {
    try {
      ...
      // API wrapper returns data directly, but keep fallback
  ...
  const fetchBranches = async () => {
    try {
      ...
      // ignore branch fetch errors for now
    }
  };

  const handleCreate = async () => {
    try {
      // Determine branch to send: chairman selects, others use their branch
      const branchToSend =
      ...
      // use returned data to optimistically update list
    ...
    <div className="space-y-4">
      ...
      {showForm && (
        <div className="p-4 border rounded bg-white">
          ...
          {/* Branch selector for chairman; for other roles we show selected branch as read-only label */}
