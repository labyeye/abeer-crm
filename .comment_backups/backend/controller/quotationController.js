...
const getAllQuotations = asyncHandler(async (req, res, next) => {
  ...
  if (startDate && endDate) {
    // Match either primary functionDetails date or any entry in functionDetailsList
    query.$or = [
...
const createQuotation = asyncHandler(async (req, res, next) => {
  ...
  // Accept either a single functionDetails or a list of functionDetailsList
  ...
  // Normalize incoming functionDetailsList if present
  ...
  const quotationData = {
    ...
    // Fill primary functionDetails from single input or from first item of list
    functionDetails: {
    ...
    services: Array.isArray(req.body.services)
      ? req.body.services.map((s) => ({
          ...
          // support incoming serviceType, type or category
...
const updateQuotation = asyncHandler(async (req, res, next) => {
  ...
  // If client provided a functionDetailsList in update, also ensure primary functionDetails is set from first entry for backwards compatibility
...
const getQuotationPdf = asyncHandler(async (req, res, next) => {
  ...
  // If multiple function details exist, list the schedule
