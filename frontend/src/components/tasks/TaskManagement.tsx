...
const TaskManagement = () => {
  ...
  const [assignEntries, setAssignEntries] = useState<
    Array<{ staffId: string; source: "branch" | "vendor"; fee?: number }>
  >([]);
  ...
  const [assignedStaffSelection, setAssignedStaffSelection] = useState<
    Array<{ staffId: string; source: "branch" | "vendor"; fee?: number }>
  >([]);
  ...
  const handleUpdateTaskStatus = async (taskId: string, status: string) => {
    try {
      ...
      if (status === "in_progress")
        payload.actualStartTime = new Date().toISOString();
    ...
    try {
      ...
      if (!selectedBookingId && assignedStaffSelection.length === 0) {
        addNotification({
          ...
          message:
            "Please select a booking or assign staff before creating a task.",
        });
        return;
      }
      const payload: any = {
        ...
        assignedStaff: assignedStaffSelection.map((a) => ({
          staffId: a.staffId,
          source: a.source,
          fee: (a as any).fee,
        })),
  ...
  const openEditModal = (task: Task) => {
    ...
    setEditWorkStartDate(
      task.workStartDate ? task.workStartDate.split("T")[0] : ""
    );
  ...
  const openAssignModal = (task: Task) => {
    ...
    setAssignEntries(
      entries.length ? entries : [{ staffId: "", source: "branch", fee: 0 }]
    );
  ...
  const handleSaveAssign = async () => {
    ...
    try {
      const payload: any = {
        assignedStaff: assignEntries
          .filter((e) => e.staffId)
          .map((e) => ({ staffId: e.staffId, source: e.source, fee: e.fee })),
      };
      ...
      addNotification({
        type: "error",
        title: "Error",
        message: err?.response?.data?.message || "Failed to update assignments",
      });
    }
  };
  ...
  const getVendorCost = (task: Task) => {
    try {
      return (task.assignedTo || []).reduce(
        (sum: number, a: any) =>
          sum + (a && a.source === "vendor" && a.fee ? Number(a.fee) : 0),
        0
      );
    } catch (e) {
      return 0;
    ...
    <div className="space-y-6">
      ...
      {taskStats && (
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          <StatCard title="Total Tasks" value={taskStats.total} icon={Calendar} />

          <StatCard title="Pending" value={taskStats.pending} icon={Clock} />

          <StatCard title="In Progress" value={taskStats.inProgress} icon={Play} />

          <StatCard title="Completed" value={taskStats.completed} icon={CheckCircle} />

          <StatCard title="Skipped" value={taskStats.skipped} icon={XCircle} />
        </div>
      )}
              ...
              <tr key={task._id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                  {task.workStartDate || task.workEndDate
                    ? `${
                        task.workStartDate
                          ? new Date(task.workStartDate).toLocaleDateString()
                          : ""
                      }${
                        task.workEndDate
                          ? " - " +
                            new Date(task.workEndDate).toLocaleDateString()
                          : ""
                      }`
                    : "No dates"}
                </td>
                    ...
                    <div>
                      {task.assignedTo.map((a: any, idx: number) => (
                        <div key={idx} className="flex items-center gap-2">
                          <span>
                            {(a.staff &&
                              (a.staff.name || a.staff.user?.name)) ||
                              (a.staff && a.staff.employeeId) ||
                              "-"}
                          </span>
                          <span className="text-xs text-gray-500">
                            ({a.source === "vendor" ? "Vendor" : "Branch"})
                          </span>
                          {a.source === "vendor" && a.fee ? (
                      ...
                      {getVendorCost(task) > 0 && (
                        <div className="text-xs text-gray-600 mt-1">
                          Vendor cost: ₹{getVendorCost(task)}
                        </div>
                      )}
                    </div>
                  ) : (
                  ...
                  try {
                    ...
                    if (booking.services && booking.services.length) {
                      booking.services.forEach((s: any, idx: number) => {
                        const typeLabel =
                          Array.isArray(s.serviceType) && s.serviceType.length
                            ? ` - ${s.serviceType.join(", ")}`
                            : "";
                    ...
                    if (
                      ...
                      booking.functionDetailsList.length
                    ) {
                      booking.functionDetailsList.forEach(
                        (f: any, idx: number) => {
                          if (f.service) {
                            const typeLabel =
                              Array.isArray(f.serviceType) &&
                              f.serviceType.length
                                ? ` - ${f.serviceType.join(", ")}`
                                : "";
                    ...
                    if (
                      ...
                      booking.functionDetailsList.forEach((fd: any) => {
                        if (fd.assignedStaff && fd.assignedStaff.length)
                          assignedIds.push(...fd.assignedStaff);
                      });
                    }
                    if (
                      assignedIds.length === 0 &&
                      booking.staffAssignment &&
                      booking.staffAssignment.length
                    ) {
                      assignedIds.push(
                        ...booking.staffAssignment.map((s: any) => s.staff)
                      );
                    }
                    setAssignedStaffSelection(
                      assignedIds.map((sid: any) => ({
                        staffId: sid,
                        source: "branch",
                      }))
                    );
              ...
              >
                ...
                {bookings.map((b) => (
                  <option key={b._id} value={b._id}>
                    {b.bookingNumber || (b.client && b.client.name) || b._id}
                  </option>
                ))}
              </select>
            </div>
            ...
            <div>
              ...
              >
                ...
                {bookingServices.map((s) => (
                  <option key={s.id} value={s.value || s.name}>
                    {s.name}
                  </option>
                ))}
              </select>
                ...
                {assignedStaffSelection.map((a, idx) => (
                  <div key={idx} className="flex gap-2 items-center">
                    <select
                      ...
                      value={a.staffId}
                      onChange={(e) => {
                        const arr = [...assignedStaffSelection];
                        arr[idx].staffId = e.target.value;
                        setAssignedStaffSelection(arr);
                      }}
                    >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <input
                        type="radio"
                        name={`source_${idx}`}
                        checked={a.source === "branch"}
                        onChange={() => {
                          const arr = [...assignedStaffSelection];
                          arr[idx].source = "branch";
                          setAssignedStaffSelection(arr);
                        }}
                      />
                      ...
                      <input
                        type="radio"
                        name={`source_${idx}`}
                        checked={a.source === "vendor"}
                        onChange={() => {
                          const arr = [...assignedStaffSelection];
                          arr[idx].source = "vendor";
                          setAssignedStaffSelection(arr);
                        }}
                      />
                      {a.source === "vendor" && (
                        <input
                          ...
                          onChange={(e) => {
                            const arr = [...assignedStaffSelection];
                            arr[idx].fee = parseFloat(e.target.value) || 0;
                            setAssignedStaffSelection(arr);
                          }}
                        />
                      )}
                      <button
                        ...
                        onClick={() => {
                          setAssignedStaffSelection(
                            assignedStaffSelection.filter((_, i) => i !== idx)
                          );
                        }}
                      >
                        Remove
      ...
      <NeomorphicModal
        ...
        title={
          assignTask
            ? `Assign Staff - ${assignTask.booking?.bookingNumber || ""}`
            : "Assign Staff"
        }
      >
        <div className="space-y-4">
          {assignEntries.map((entry, idx) => (
            <div key={idx} className="flex gap-2 items-center">
              <select
                ...
                value={entry.staffId}
                onChange={(e) => {
                  const arr = [...assignEntries];
                  arr[idx].staffId = e.target.value;
                  setAssignEntries(arr);
                }}
              >
                <option value="">Select staff</option>
                {staffList.map((s) => (
              ...
              <div className="flex items-center gap-2">
                ...
                <input
                  type="radio"
                  name={`assign_source_${idx}`}
                  checked={entry.source === "branch"}
                  onChange={() => {
                    const arr = [...assignEntries];
                    arr[idx].source = "branch";
                    setAssignEntries(arr);
                ...
                <button
                  className="px-2 py-1 bg-red-500 text-black rounded"
                  onClick={() =>
                    setAssignEntries(assignEntries.filter((_, i) => i !== idx))
                  }
                >
                  Remove
                </button>
