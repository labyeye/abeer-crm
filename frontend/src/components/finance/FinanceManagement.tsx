...
const FinanceManagement = () => {
    ...
    const load = async () => {
      try {
        ...
        const totalRevenue = (data.revenue || []).reduce(
          (s: number, r: any) => s + (r.totalRevenue || 0),
          0
        );
        ...
        try {
          ...
          const taggedDaily = (dailyData || []).map((d: any) => ({
            ...d,
            isDailyExpense: true,
          }));
        ...
        const ensureBranchCached = async (branchId: string) => {
          ...
          try {
            ...
            setBranches((prev) =>
              prev.find((x: any) => x._id === branchId) ? prev : [...prev, b]
            );
          } catch (err) {
        ...
        const resolveBranchName = (branch: any) => {
          ...
          if (typeof branch === "string") {
            const b =
              branchCache[branch] ||
              (branches || []).find((br) => br._id === branch);
        ...
        await Promise.all(
          (expData || []).map(async (ex: any) => {
            const bid =
              typeof ex.branch === "string" ? ex.branch : ex.branch?._id;
            if (
              bid &&
              !((branches || []).find((b) => b._id === bid) || branchCache[bid])
            ) {
        ...
        const md = months.map((m) => {
          const income = bookingsData.reduce((sum: number, b: any) => {
            // Only count income for bookings that are completed
            if (b.status !== "completed") return sum;
            const bd = b.functionDetails?.date
              ? new Date(b.functionDetails.date)
              : new Date(b.createdAt || b.pricing?.createdAt);
            return bd.getFullYear() === m.date.getFullYear() &&
              bd.getMonth() === m.date.getMonth()
              ? sum + (b.pricing?.totalAmount || 0)
              : sum;
          }, 0);
          const exp = expData.reduce((sum: number, ex: any) => {
            const ed = ex.expenseDate
              ? new Date(ex.expenseDate)
              : new Date(ex.createdAt);
            return ed.getFullYear() === m.date.getFullYear() &&
              ed.getMonth() === m.date.getMonth()
              ? sum + (ex.amount || 0)
              : sum;
          }, 0);
        ...
        expData
          .filter((ex: any) => !ex.isDailyExpense)
          .forEach((ex: any) => {
            const c = ex.category || ex.purpose || "other";
            catMap[c] = (catMap[c] || 0) + (ex.amount || 0);
          });
        ...
        const totalExp =
          Object.values(catMap).reduce((s: number, v: any) => s + v, 0) || 1;
        const cats = Object.keys(catMap).map((k) => ({
          category: k,
          amount: catMap[k],
          percentage: Math.round((catMap[k] / totalExp) * 100),
        }));
        ...
        const pending = bookingsData
          .filter(
            (b: any) =>
              (b.pricing?.remainingAmount || 0) > 0 ||
              (b.paymentStatus && b.paymentStatus !== "completed")
          )
          .map((b: any) => ({
            id: b._id,
            client: b.client?.name || "",
            amount:
              b.pricing?.remainingAmount ||
              b.pricing?.totalAmount - b.pricing?.advanceAmount ||
              0,
            dueDate: b.functionDetails?.date,
          }));
        ...
        const txs = [
          ...expData.map((ex: any) => ({
            _id: ex._id,
            type: "expense",
            title: ex.title || ex.purpose || ex.category,
            amount: ex.amount,
            date: ex.expenseDate || ex.createdAt,
            status: ex.status,
            vendor: ex.vendor,
          ...
          ...bookingsData.map((b: any) => ({
            _id: b._id,
            type: "income",
            title: b.serviceNeeded || b.bookingNumber,
            amount: b.pricing?.totalAmount || 0,
            date: b.functionDetails?.date || b.createdAt,
            status: b.status,
            client: b.client?.name,
          })),
        ].sort(
  ...
  const onBranchChange = async (branchId: string) => {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "Failed to fetch daily expenses for filtered finance view",
          err
        );
      }
      ...
      const md = months.map((m) => {
        const income = bookingsData.reduce((sum: number, b: any) => {
          // Only include completed bookings in income calculation
          if (b.status !== "completed") return sum;
          const bd = b.functionDetails?.date
            ? new Date(b.functionDetails.date)
            : new Date(b.createdAt || b.pricing?.createdAt);
      ...
      addNotification({
        type: "error",
        title: "Error",
        message: "Failed to filter by branch",
      });
    } finally {
      setLoadingFinance(false);
    }
  ...
  // Ensure maxAmount is at least 1 to avoid division by zero and NaN widths
  ...
  (revenueByBranch || []).forEach((r: any) =>
    branchKeysSet.add(r._id || r.branch)
  );
  (expensesByBranch || []).forEach((e: any) =>
    branchKeysSet.add(e._id || e.branch)
  );
                ...
                <div className="mt-6">
                  ...
                  <div className="space-y-2 text-sm text-gray-700">
                    {branchKeys.map((key: any) => {
                      const rb =
                        (revenueByBranch || []).find(
                          (r: any) => (r._id || r.branch) === key
                        ) || {};
                      const expObj =
                        (expensesByBranch || []).find(
                          (e: any) => (e._id || e.branch) === key
                        ) || {};
                      const expVal = expObj
                      ...
                      return (
                        <div key={key} className="flex justify-between">
                          <div>
                            {branches.find((b) => b._id === key)?.name ||
                              rb.name ||
                              key}
                          </div>
                          <div>
                      ...
                      <div className="flex items-center">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-500">
                            {new Date(
                              tx.date || tx.expenseDate || tx.createdAt
                            ).toLocaleDateString()}
                          </p>
                          {tx.branchName && (
                ...
                <div className="bg-gray-50 rounded-lg p-4">
                  ...
                  <div className="space-y-3 text-sm">
                    <p className="text-gray-600">
                      • Equipment purchases account for 35% of total expenses
                    </p>
                    <p className="text-gray-600">
                      • Staff costs have increased by 12% this quarter
                    </p>
                    <p className="text-gray-600">
                      • Marketing spend is below industry average
                    </p>
                    <p className="text-gray-600">
                      • Consider bulk equipment purchases for better rates
                    </p>
                  </div>
                </div>
              </div>
